...
- 外部复核默认走 repo 外 OpenRouter 配置，作为第二裁判而不是主执行模型
...
- 已有远程服务器总管画像：
  ...
  - OpenRouter 免费模型负责计划复核、压缩复核和约束提炼
  - 远端默认不把 `codex` 放进主执行链
- 已有项目注册表原型：
  - `project register`
  - `project list`
...
- 已能走第一版总管 intake 闭环：
  ...
  - 飞书统一会话由后台维护循环巡检，并在每 5 小时间隔到点后自动压缩上下文
  - 前台记忆写入和后台压缩会互斥，避免重复提取
  - 计划和压缩结果都可走外部第二模型复核，并把约束写回长期记忆
  - 自动回一版中文确认摘要
