...
test('profile show returns the remote server manager defaults', async () => {
  ...
  assert.equal(payload.profile.channels.coworker.authority, 'advisory_only')
  assert.equal(payload.profile.channels.coworker.auto_execute_allowed, false)
  assert.equal(payload.profile.registry_policy.auto_discovery_write_requires_confirmation, true)
